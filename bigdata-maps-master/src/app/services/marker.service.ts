...
export class MarkerService {
  ...
  makeEscuelaMarkers(map: L.map, idestado, idmunicipio, actividad): void {
   
    this.dataApiService.getEscuelascompleto(idestado, idmunicipio, actividad).subscribe((res: any) => {
      for (const c of res) {
        const lat = parseFloat(c.lat);
        const lon = parseFloat(c.lng);
        const marker = L.marker([lat, lon],
...
makeComercioMarkers(map: L.map, idestado, idmunicipio, actividad): void {
  ...
  this.dataApiService.getComerciosCompleto(idestado, idmunicipio, actividad).subscribe((res: any) => {
    for (const c of res) {
      const lat = parseFloat(c.lat);
      const lon = parseFloat(c.lng);
      const marker = L.marker([lat, lon],
...
makeHospitalMarkers(map: L.map, idestado, idmunicipio, actividad): void {
  ...
  this.dataApiService.getHospitalesCompleto(idestado, idmunicipio, actividad).subscribe((res: any) => {
    for (const c of res) {
      const lat = parseFloat(c.lat);
      const lon = parseFloat(c.lng);
      const marker = L.marker([lat, lon],
